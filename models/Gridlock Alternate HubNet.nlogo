...
to go
  ...
  [
    ...
    ask players [
      ...
      if metric-code' != metric-code [

        set metric-code' metric-code

        set metric-value ""

        set my-plot-metric metric-code
        set my-plot-data   []
...
to update-list-info
  ...
  ask intersections with [ my-player != nobody and [my-plot-metric] of my-player != "" ]
...
to add-new-client-metric-value  ;; intersection procedure
  ...
  let new-value "bad default value"
  ...
  [
    set metric-error (word my-plot-metric " does not produce a numeric value.")
    clear-client-plot-data
  ]
...
to set-current-client-metric-player [ player-to-plot ]
  ...
  set current-client-metric [my-plot-metric] of player-to-plot
end
...
to-report user-id-of-intersection-for-current-metric
  ifelse current-player-for-client-metric = nobody or not is-patch? current-player-for-client-metric
  [ report "" ]
  [ report [username] of [my-player] of current-player-for-client-metric ]
end
